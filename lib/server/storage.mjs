...
export async function addUserToIndex(email, timestamp) {
  await redis.zadd(USER_INDEX_KEY, { score: timestamp, member: email });
}

export async function getUserCount() {
  return (await redis.zcard(USER_INDEX_KEY)) ?? 0;
}

export async function getRecentUsers(count = 30) {
  return redis.zrange(USER_INDEX_KEY, 0, count - 1, { rev: true });
}

export async function batchGetAccounts(emails) {
  if (!emails?.length) return [];
  const keys = emails.map((e) => `${ACCOUNT_PREFIX}${e}`);
