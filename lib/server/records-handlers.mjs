import { getRecords, setRecords, consumeRateLimit } from "./storage.mjs";
...
export async function handleRecordsSync(req, res) {
  ...
  // IP 기반 rate limit: 1분에 30회
  const ip = clientIp(req);
  if (!(await consumeRateLimit(`records:sync:${ip}`, 60, 30))) {
    return res.status(429).json({ error: "too_many_attempts", message: "잠시 후 다시 시도해 주세요." });
  }
...
export async function handleRecordsSave(req, res) {
  ...
  // IP 기반 rate limit: 1분에 60회
  const ip = clientIp(req);
  if (!(await consumeRateLimit(`records:save:${ip}`, 60, 60))) {
    return res.status(429).json({ error: "too_many_attempts", message: "잠시 후 다시 시도해 주세요." });
  }
