...
import { sendPaymentApprovedEmail } from "./email.mjs";

const ADMIN_SECRET = process.env.ADMIN_SECRET ?? "";

function checkAdminAuth(req) {
  if (!ADMIN_SECRET) return false;
  // === 단순 비교 대신 타이밍 공격에 안전한 constantTimeEqual 사용
  return constantTimeEqual(bearerToken(req), ADMIN_SECRET);
}
...
export async function handleAdminDeleteUser(req, res) {
  ...
  // 계정 + 인증 정보 + 기록 + 세션 동시 삭제 (고아 데이터 방지)
  await Promise.all([
    deleteAccount(email),
    deleteVerification(email),
    account.id ? deleteRecords(account.id) : Promise.resolve(),
    account.id ? deleteUserSessions(account.id) : Promise.resolve()
  ]);
...
export async function handleAdminApprovePayment(req, res) {
  ...
  // 승인 이메일 발송 (실패해도 승인 자체는 성공으로 처리)
  sendPaymentApprovedEmail({ to: email, name: account.name, tier: account.tier })
    .catch((err) => console.error("[approve-payment] email failed:", err?.message ?? err));
...
export async function handleAdminForceLogout(req, res) {
  if (!checkAdminAuth(req)) return res.status(401).json({ error: "unauthorized" });
  ...
  const account = await getAccount(email);
  if (!account) return res.status(404).json({ error: "user_not_found" });
  if (!account.id) return res.status(400).json({ error: "invalid_account" });

  const count = await deleteUserSessions(account.id);
  return res.status(200).json({ ok: true, sessionsDeleted: count });
}
